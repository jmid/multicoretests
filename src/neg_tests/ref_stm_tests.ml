...
module RT_int_Dom   = STM_Domain.Make(RConf_int)
module RT_int64_Seq = STM_Seq.Make(RConf_int64)
module RT_int64_Dom = STM_Domain.Make(RConf_int64)

module RConf_int_GC = STM.AddGC(RConf_int)
module RConf_int64_GC = STM.AddGC(RConf_int64)

module RT_int_GC_Seq = STM_Seq.Make(RConf_int_GC)
module RT_int_GC_Dom = STM_Domain.Make(RConf_int_GC)
module RT_int64_GC_Seq = STM_Seq.Make(RConf_int64_GC)
module RT_int64_GC_Dom = STM_Domain.Make(RConf_int64_GC)
;;
...
QCheck_runner.run_tests_main
  (let count = 1000 in
   [RT_int_Seq.agree_test        ~count ~name:"global int ref test";
    RT_int_Dom.agree_test_par    ~count ~name:"global int ref test";
    RT_int_GC_Dom.agree_test_par ~count ~name:"global int ref test (w/AddGC functor)";
    RT_int_Seq.agree_test        ~count ~name:"global int64 ref test";
    RT_int_Dom.agree_test_par    ~count ~name:"global int64 ref test";
    RT_int_GC_Dom.agree_test_par ~count ~name:"global int64 ref test (w/AddGC functor)";
   ])
